...
output "docker_pull_command" {
  ...
  value       = "docker pull ${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/ecr-public/nginx/nginx:stable-alpine"
}

output "docker_build_command" {
  description = "Docker build command"
  value       = "docker build -t ${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/app/${var.app_name}:latest ./app"
}

output "docker_push_command" {
  description = "Docker push command (will create repository via create on push)"
  value       = "docker push ${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com/app/${var.app_name}:latest"
}
